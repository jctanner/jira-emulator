...
async def register_dynamic(
    body: dict, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
...
async def list_dynamic(
    ...
    maxResults: int = Query(100, ge=1, le=100),
    ...
    rows = (
        (
            await db.execute(
                select(Webhook)
                .where(Webhook.kind == "dynamic", Webhook.owner_user_id == current_user.id)
                .order_by(Webhook.id)
            )
        )
        .scalars()
    ...
    return {
        ...
        "total": len(values),
        "isLast": startAt + maxResults >= len(values),
        "values": values[startAt : startAt + maxResults],
    }
        ...
        (
            await db.execute(
                select(Webhook).where(
                    Webhook.id.in_(ids), Webhook.kind == "dynamic", Webhook.owner_user_id == current_user.id
                )
            )
        )
        .scalars()
    ...
    rows = (
        (
            await db.execute(
                ...
                .where(WebhookOutbox.owner_user_id == current_user.id, WebhookOutbox.state == "failed")
                .order_by(WebhookOutbox.failed_at)
                .limit(maxResults + 1)
            )
        )
        .scalars()
    ...
    values = [
        {
            ...
            "body": row.payload,
            "url": row.url,
            "failureTime": int((row.failed_at or row.created_at).timestamp() * 1000),
        }
        for row in rows[:maxResults]
    ]
    result = {"values": values, "maxResults": maxResults}
    if len(rows) > maxResults and values:
        result["next"] = (
            f"{get_settings().BASE_URL}/rest/api/2/webhook/failed?after={values[-1]['failureTime']}&maxResults={maxResults}"
        )
    return result
...
async def get_admin_webhook(
    webhook_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
...
async def update_admin_webhook(
    webhook_id: int, body: dict, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    row = await db.get(Webhook, webhook_id)
    if row is None or row.kind != "admin":
        raise ValueError(f"Webhook {webhook_id} not found")
