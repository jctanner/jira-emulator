...
async def _create_issue(client, project="RHOAIENG", summary="Test", **extra_fields):
    fields = {"project": {"key": project}, "summary": summary, "issuetype": {"name": "Bug"}}
...
async def test_create_issue_with_unknown_custom_field_string(client):
    ...
    issue = await _create_issue(client, summary="CF auto-create", customfield_99999="hello")
...
async def test_create_issue_with_unknown_custom_field_json(client):
    ...
    adf = {
        ...
        "content": [
            {
                "type": "paragraph",
                "content": [{"type": "inlineCard", "attrs": {"url": "https://github.com/org/repo/pull/42"}}],
            }
        ],
    }
    issue = await _create_issue(client, summary="CF ADF", customfield_88888=adf)
    resp = await client.get(f"/rest/api/2/issue/{issue['key']}", headers=AUTH)
    fields = resp.json()["fields"]
...
async def test_import_with_raw_customfield_key(client):
    ...
    adf = {
        ...
        "content": [
            {
                "type": "paragraph",
                "content": [{"type": "inlineCard", "attrs": {"url": "https://github.com/org/repo/pull/55"}}],
            }
        ],
    }
...
async def test_create_issue_with_number_custom_field(client):
    ...
    issue = await _create_issue(client, summary="CF number", customfield_66666=42.5)
    resp = await client.get(f"/rest/api/2/issue/{issue['key']}", headers=AUTH)
