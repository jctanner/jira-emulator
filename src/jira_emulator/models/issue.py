...
class Issue(Base):
    ...
    issue_type_id: Mapped[int] = mapped_column(Integer, ForeignKey("issue_types.id"), nullable=False)
    summary: Mapped[str] = mapped_column(Text, nullable=False)
    description: Mapped[str | None] = mapped_column(Text)
    status_id: Mapped[int] = mapped_column(Integer, ForeignKey("statuses.id"), nullable=False)
    priority_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("priorities.id"))
    resolution_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("resolutions.id"))
    ...
    component_associations: Mapped[list["IssueComponent"]] = relationship(cascade="all, delete-orphan")
    fix_version_associations: Mapped[list["IssueFixVersion"]] = relationship(cascade="all, delete-orphan")
    affects_version_associations: Mapped[list["IssueAffectsVersion"]] = relationship(cascade="all, delete-orphan")
    custom_field_values: Mapped[list["IssueCustomFieldValue"]] = relationship(cascade="all, delete-orphan")
    watchers: Mapped[list["Watcher"]] = relationship(cascade="all, delete-orphan")
    sprint_associations: Mapped[list["IssueSprint"]] = relationship(cascade="all, delete-orphan")
    outward_links: Mapped[list["IssueLink"]] = relationship(
        foreign_keys="IssueLink.outward_issue_id", cascade="all, delete-orphan", overlaps="outward_issue"
    )
    inward_links: Mapped[list["IssueLink"]] = relationship(
        foreign_keys="IssueLink.inward_issue_id", cascade="all, delete-orphan", overlaps="inward_issue"
    )
...
from jira_emulator.models.version import IssueAffectsVersion, IssueFixVersion  # noqa: E402, F401
