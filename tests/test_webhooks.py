...
async def test_admin_webhook_lifecycle(client: httpx.AsyncClient):
    created = await client.post(
        ...
        json={"name": "Demo", "url": "https://example.test/hook", "events": ["jira:issue_created"],
              "secret": "top-secret"},
        headers=AUTH_HEADER,
    )
    ...
    insecure = await client.post(
        "/rest/webhooks/1.0/webhook",
        json={"name": "Local", "url": "https://example.test/local", "events": ["jira:issue_created"],
...
async def test_dynamic_webhook_v3_registration_and_refresh(client: httpx.AsyncClient):
    registered = await client.post(
        "/rest/api/3/webhook",
        json={"url": "https://example.test/hook", "webhooks": [
    ...
    removed = await client.request("DELETE", "/rest/api/3/webhook", json={"webhookIds": [webhook_id]}, headers=AUTH_HEADER)
