...
async def upload_attachments(
    ...
    if issue is None:
        raise HTTPException(
            status_code=404,
            detail={"errorMessages": [f"Issue Does Not Exist: {issueIdOrKey}"], "errors": {}},
        )
    ...
    for upload in files:
        ...
        result = await db.execute(
            select(Attachment).options(selectinload(Attachment.author)).where(Attachment.id == attachment.id)
        )
...
async def get_attachment_meta(
    ...
    result = await db.execute(
        select(Attachment).options(selectinload(Attachment.author)).where(Attachment.id == attachment_id)
    )
...
async def get_attachment_content(
    ...
    if not os.path.exists(disk_path):
        raise HTTPException(
            status_code=404,
            detail={"errorMessages": ["Attachment file missing from disk"], "errors": {}},
        )
...
async def get_attachment_thumbnail(
    ...
    if att is None:
        raise HTTPException(
            status_code=404,
            detail={"errorMessages": ["Attachment not found"], "errors": {}},
        )

    if att.mime_type not in _IMAGE_TYPES:
        raise HTTPException(
            status_code=404,
            detail={"errorMessages": ["No thumbnail available for non-image attachment"], "errors": {}},
        )
